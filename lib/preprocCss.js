...
options = {
  ...
  preprocMapName: '.preproc_map.json'
};
...
function getMap(pathToJcrRoot, callback) {
  if (map) {
    ...
    fs.readFile(pth.join(pathToJcrRoot, options.preprocMapName), function (err, file) {
      if (err){
        callback(err);
      } else {
        map = JSON.parse(file.toString());
        callback(null, map);
      }
    });
  }
  ...
  getMap(pathToJcrRoot, function (err, map) {
    if (err) {
      ...
      if (map[pathWithoutJcr]) {
        callback(null, pth.join(pathToJcrRoot, map[pathWithoutJcr]));
      } else {
...
function setJcrRoot(jsrRoot) {
  options.matchJsrRoot = jsrRoot;
}
...
module.exports = {
  /**
   * Set reg exp for root directory
   * @param {regexp} root directory's mtacher
   */
  setJcrRoot: setJcrRoot,
  /**
